...
if __name__ == "__main__":
    ...
    for i in range(num_iterations):
        ...
        # traj_info, dones, episodic_rewards, episodic_returns = simulate_transitions(
        #     envs, agent, policy=lambda x: policy(x, agent, params), horizon=max_timesteps
        # )

        # For perturbed policy
        # traj_info_ = None
        # dones_ = None
        # nu = None
        # u = None
        ...
        # if False:
        #     nu = 0.1
        #     u = np.random.randn(params.shape[0],)
        #     params_ = params + nu*u
        #     traj_info_, dones_, episodic_rewards_, episodic_returns_ = simulate_trajectories(
        #         envs, agent, policy=lambda x: policy(x, agent, params_), horizon=max_timesteps
        #     )
        t1 = time.time() - t_
