...
def acrpn(params: List[Tensor],
    ...
    grad_estimates = list(torch.autograd.grad(l1[0].sum(), params, create_graph=True))
    # grad_estimates_ = list(torch.autograd.grad(l1[1].mean(), params, create_graph=True))
    ...
    hvp_func = lambda v: _estimate_hvp(params, v, l1[0], uJp=uJp, u=u, grad_estimates=grad_estimates)
...
def _estimate_hvp(params: List[Tensor],
    ...
    hvp1 = list(torch.autograd.grad(l1, params, grad_outputs=Jvp, retain_graph=True))
